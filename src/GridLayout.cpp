...
void GridLayout::onWindowCreatedTiling(CWindow *pWindow, eDirection direction) {
  ...
  const auto pActiveWorkspace = g_pCompositor->getWorkspaceByID(
...
void GridLayout::calculateWorkspace(const int &ws) {
  ...
  const auto pMonitor =
      g_pCompositor->getMonitorFromID(pWorksapce->m_iMonitorID);
  ...
  static const auto *GAPPO =
      &HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprshell:overview_gappo")
           ->intValue;
  static const auto *GAPPI =
      &HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprshell:overview_gappi")
           ->intValue;
...
void GridLayout::changeToActivceSourceWorkspace() {
  ...
  // g_pEventManager->postEvent(SHyprIPCEvent{"workspace",
  // pWorksapce->m_szName}); EMIT_HOOK_EVENT("workspace", pWorksapce);
  // g_pCompositor->focusWindow(pWindow);
}
...
void GridLayout::moveWindowToSourceWorkspace() {
  ...
  for (auto &nd : m_lGridNodesData) {
    if (nd.pWindow &&
        (nd.pWindow->m_iWorkspaceID != nd.ovbk_windowWorkspaceId ||
      ...
      if (!pWorkspace)
        pWorkspace = g_pCompositor->createNewWorkspace(
            nd.ovbk_windowWorkspaceId, nd.pWindow->m_iMonitorID,
...
void GridLayout::onDisable() { m_lGridNodesData.clear(); }