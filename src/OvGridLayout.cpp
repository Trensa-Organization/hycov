...
OvGridLayout::getOldLayoutRecordNodeFromWindow(CWindow *pWindow) {
...
void OvGridLayout::onWindowCreatedTiling(CWindow *pWindow,
  ...
  const auto pActiveWorkspace =
      g_pCompositor->getWorkspaceByID(pTargetMonitor->activeWorkspace);
...
void OvGridLayout::onWindowRemovedTiling(CWindow *pWindow) {
  ...
  if (pNode->isGroupActive && pNode->pGroupPrevWindow &&
      pNode->pGroupPrevWindow != pNode->pWindow) {
...
void OvGridLayout::changeToActivceSourceWorkspace() {
  ...
  if (pNode) {
    ...
    pWorksapce = g_pCompositor->getWorkspaceByID(
  ...
  for (auto &nd : m_lOvGridNodesData) {
    if (nd.pWindow &&
      ...
      if (!pWorkspace) {
        ...
        hycov_log(LOG, "create workspace: id:{} monitor:{} name:{}",
